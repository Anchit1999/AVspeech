...
def cut_video(raw_video_path, name, video_id, start, end):
	"""
	...
	for st, en in zip(start, end):
		slice_path = os.path.join(vid_dir, video_id + "_" + str(st) + "_" + str(en) + ".mp4")
		st = format_time(st)
		en = format_time(en)
		return_code = subprocess.call(["ffmpeg", "-loglevel", "quiet", "-i", raw_video_path, "-strict", "-2",
										"-ss", str(st), "-to", str(en), slice_path])
		print(slice_path)
		success += return_code == 0
...
def process_video(video_id, directory, start, end, video_format="mp4", compress=False, overwrite=False, log_file=None):
	...
	name = os.path.join(directory, video_id)
	...
	success = cut_video(download_path, name, video_id, start, end)
...
def download_class_parallel(videos_dict, directory, videos_queue):
	"""
